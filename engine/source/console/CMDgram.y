...
fn_decl_stmt
   : rwDEFINE IDENT 
     ...
     '(' var_list_decl ')' '{' statement_list '}'
      { $$ = FunctionDeclStmtNode::alloc( $1.lineNumber, $2.value, NULL, $5, $8 );  FunctionDeclStmtNode::popVariableScope(); }
    | rwDEFINE IDENT opCOLONCOLON IDENT
     {  FunctionDeclStmtNode::pushVariableScope(); } 
     '(' var_list_decl ')' '{' statement_list '}'
     { $$ = FunctionDeclStmtNode::alloc( $1.lineNumber, $4.value, $2.value, $7, $10 );  FunctionDeclStmtNode::popVariableScope(); }
   ;
...
object_name
   :
      { $$ = StrConstNode::alloc( CMDGetCurrentLine(), "", false); }
   | expr
      { $$ = $1; }
   ;
